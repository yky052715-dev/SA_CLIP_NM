...
CANDIDATE_CONFIG="${CANDIDATE_CONFIG:-${PROJECT_ROOT}/configs/mvtec_dev5_highres_clip_adaptive_area_component.yaml}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${PROJECT_ROOT}/outputs/localization_refinement/highres_adaptive_area_component_seed42}"
SEED="${SEED:-42}"
...
run_case() {
    ...
    echo "[highres-adaptive-area-component] ${case_name}"
...
run_case "H1_clip336_adaptive_area_component" "${CANDIDATE_CONFIG}" 336 0.0444444444
...
cases = ["H0_clip224", "H1_clip336_adaptive_area_component"]
metrics = [
    "pixel_AUROC",
    "pixel_F1_calibrated",
    "pixel_F1_oracle",
...
print("\n# H1 adaptive-area + component postprocess seed42")
print("\n## Mean metrics")
...
print("\n## H1 per-category")
print("| category | F1 | oracle | overseg | recall | normal_fp | q | min_area_px | area_p95_limit |")
print("|---|---:|---:|---:|---:|---:|---:|---:|---:|")
for row in summaries[cases[1]]["categories"]:
    print(
        ...
        f"| {row.get('adaptive_selected_pixel_image_quantile')} "
        f"| {row.get('postprocess_min_component_area_pixels')} "
        f"| {row.get('adaptive_max_normal_positive_area_p95_fraction')} |"
    )
PY

echo "[highres-adaptive-area-component] complete: ${OUTPUT_ROOT}"
